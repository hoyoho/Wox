...
/* We basically follow the Json-RPC 2.0 spec (http://www.jsonrpc.org/specification) to invoke methods between Wow and other plugins, 
 ...
 *      Wow -------JsonRPCServerRequestModel--------> client
 *      Wow <------JsonRPCQueryResponseModel--------- client
 *      
 * When execute a action (which mean user select an item in reulst item):
 *      Wow -------JsonRPCServerRequestModel--------> client
 *      Wow <------JsonRPCResponseModel-------------- client
 * 
 */
...
namespace Wow.Core.Plugin
{
    ...
    /// <summary>
    /// Json RPC Request that Wow sent to client
    /// </summary>
    ...
    /// <summary>
    /// Json RPC Request(in query response) that client sent to Wow
    /// </summary>
    ...
    /// <summary>
    /// Represent the json-rpc result item that client send to Wow
    /// Typically, we will send back this request model to client after user select the result item
    /// But if the request method starts with "Wow.", we will invoke the public APIs we expose.
    /// </summary>
