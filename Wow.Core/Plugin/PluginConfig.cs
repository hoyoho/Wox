...
namespace Wow.Core.Plugin
{
    ...
    internal abstract class PluginConfig
    {
        ...
        private static void ParsePluginConfigs(IEnumerable<string> directories)
        {
            ...
            foreach (var directory in directories)
            {
                if (File.Exists(Path.Combine(directory, "NeedDelete.txt")))
                {
                    ...
                    {
                        Logger.WowError($"Can't delete <{directory}>", e);
                    }
                }
                else
                {
        ...
        private static PluginMetadata GetPluginMetadata(string pluginDirectory)
        {
            ...
            {
                Logger.WowError($"Didn't find config file <{configPath}>");
                return null;
            }
            ...
            catch (Exception e)
            {
                ...
                Logger.WowError($"invalid json for config <{configPath}>", e);
                return null;
            }
            ...
            {
                Logger.WowError($"Invalid language <{metadata.Language}> for config <{configPath}>");
                return null;
            }

            if (!File.Exists(metadata.ExecuteFilePath))
            {
                Logger.WowError($"execute file path didn't exist <{metadata.ExecuteFilePath}> for conifg <{configPath}");
                return null;
            }
