        ...
        public async Task UpdateApp(bool silentIfLatestVersion = true, bool updateToPrereleases = false)
        {
            try
            {
                using (UpdateManager updateManager = await GitHubUpdateManager(GitHubRepository, updateToPrereleases))
                {
                    ...
                    {
                        Logger.WowError($"Check your connection and proxy settings to api.github.com. {e.Message}");
                    ...
                    Logger.WowInfo($"Future Release <{newUpdateInfo.FutureReleaseEntry.Formatted()}>");

                    if (newReleaseVersion <= currentVersion)
                    {
                        if (!silentIfLatestVersion)
                            MessageBox.Show("You already have the latest Wow version");
                        updateManager.Dispose();
                        return;
                    }
                    ...
                    {
                        Logger.WowError($"Check your connection and proxy settings to github-cloud.s3.amazonaws.com. {e.Message}");
                        updateManager.Dispose();
                        return;
                    }
                    ...
                    Logger.WowInfo($"Update success:{newVersionTips}");
            ...
            {
                Logger.WowError($"Please check your connection and proxy settings {e.Message}");
            }
            catch (Exception e)
            {
                Logger.WowError($"cannot check update {e.Message}");
            }
