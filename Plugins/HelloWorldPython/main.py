...
class HelloWorld(Wow):
    ...
    def open_webpage(self, url, notification=True):
        """Trivial implementation of bookmarks in Wow

        Demonstrates how optional parameters in Json RPC are handled.
        """
        if notification:
            WowAPI.show_msg('Heads up', 'We opened in new window')
            webbrowser.open_new(url)
        else:
            webbrowser.open(url)

    def show_notification(self, site, extra_message):
        """Custom handler that demonstrates how to call Wow api"""
        title = 'You chose {}'.format(site)
        sub_title = 'This the extra message you sent: {}'.format(extra_message)
        ico_path = ''

        # Wrappers for many common APIs are found in JsonRPC/wow.py
        # WowAPI.show_msg(title, sub_title)

        # But you can invoke any arbitrary function in Wow dlls with JSON RPC
        print(json.dumps(
            {
                "method": "Wow.ShowMsg",
