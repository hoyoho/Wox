...
using Wow.Plugin.Program.Programs;
using Wow.Plugin.Program.Views;
using System.Threading;

namespace Wow.Plugin.Program
{
    public class Main : ISettingProvider, IPlugin, IPluginI18n, IContextMenu, ISavable, IReloadable
    {
        ...
        private static void preloadPrograms()
        {
            ...
            Logger.WowInfo($"Number of preload win32 programs <{_win32s.Length}>");
            Logger.WowInfo($"Number of preload uwps <{_uwps.Length}>");
        }
        ...
        public List<Result> Query(Query query)
        {
            ...
            if (_updateSource != null && !_updateSource.IsCancellationRequested)
            {
                ...
                Logger.WowDebug($"cancel init {_updateSource.Token.GetHashCode()} {Thread.CurrentThread.ManagedThreadId} {query.RawQuery}");
        ...
        public static void IndexPrograms()
        {
            ...
            Logger.WowInfo($"Number of indexed win32 programs <{_win32s.Length}>");
            foreach (var win32 in _win32s)
            {
                Logger.WowDebug($" win32: <{win32.Name}> <{win32.ExecutableName}> <{win32.FullPath}>");
            }
            Logger.WowInfo($"Number of indexed uwps <{_uwps.Length}>");
            foreach (var uwp in _uwps)
            {
                Logger.WowDebug($" uwp: <{uwp.DisplayName}> <{uwp.UserModelId}>");
            }
