    ...
    public class UWP
    {
        ...
        public static Application[] All()
        {
            ...
            Parallel.ForEach(PackageFoldersFromRegistry(), (package, state) =>
            {
                ...
                catch (Exception e)
                {
                    ...
                    Logger.WowError($"Cannot parse UWP {package.Location}", e);
                }
            }
            );
            return bag.ToArray();
        ...
        public class Application : IProgram
        {
            ...
            internal string ResourcesFromPri(string packageFullName, string packageName, string resourceReference)
            {
                ...
                string result = "";
                Logger.WowTrace($"package: <{packageFullName}> res ref: <{resourceReference}>");
                if (!string.IsNullOrWhiteSpace(resourceReference) && resourceReference.StartsWith(prefix))
                {
                    ...
                    Logger.WowTrace($"resourceReference {resourceReference} parsed <{parsed}> package <{packageFullName}>");
                    try
                    {
                ...
                Logger.WowTrace($"package: <{packageFullName}> pri resource result: <{result}>");
                return result;
            }
            ...
            private string PathFromUri(string packageFullName, string packageName, string packageLocation, string fileReference)
            {
                ...
                Logger.WowTrace($"package: <{packageFullName}> file ref: <{fileReference}>");
                ...
                else
                {
                    ...
                    try
                    {
                        ...
                        Logger.WowTrace($"package: <{packageFullName}> pri file result: <{result}>");
                        return result;
                    }
                    catch (Exception e)
                    {
            ...
            {
                Logger.WowTrace($"package: <{packageFullName}> pri parsed: <{parsed}>");
                ...
                if (hResult == Hresult.Ok)
                {
                    ...
                    {
                        Logger.WowError($"Can't load null or empty result pri {source} in uwp location {Package.Location}");
                        return string.Empty;
                    }
                }
                else
                {
                    ...
                    Logger.WowError($"Load pri failed {source} location {Package.Location}", e);
                    return string.Empty;
                }
            }
            ...
            private BitmapImage ImageFromPath(string path)
            {
                ...
                {
                    Logger.WowError($"|Unable to get logo for {UserModelId} from {path} and located in {Package.Location}");
            ...
            private ImageSource PlatedImage(BitmapImage image)
            {
                if (!string.IsNullOrEmpty(BackgroundColor) && BackgroundColor != "transparent")
                {
                    ...
                    {
                        Logger.WowError($"Unable to convert background string {BackgroundColor} to color for {Package.Location}");
                        return new BitmapImage(new Uri(Constant.ErrorIcon));
                    }
                }
                else
