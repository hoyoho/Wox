...
namespace Wow.Plugin.WebSearch
{
    public partial class SearchSourceSettingWindow
    {
        ...
        private void OnConfirmButtonClick(object sender, RoutedEventArgs e)
        {
            ...
            {
                var warning = _api.GetTranslation("wow_plugin_websearch_input_title");
                MessageBox.Show(warning);
            }
            else if (string.IsNullOrEmpty(_searchSource.Url))
            {
                var warning = _api.GetTranslation("wow_plugin_websearch_input_url");
                MessageBox.Show(warning);
            }
            else if (string.IsNullOrEmpty(_searchSource.ActionKeyword))
            {
                var warning = _api.GetTranslation("wow_plugin_websearch_input_action_keyword");
                MessageBox.Show(warning);
            }
