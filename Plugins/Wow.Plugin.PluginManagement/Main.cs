...
using Wow.Infrastructure.Http;
using Wow.Infrastructure.Logger;

namespace Wow.Plugin.PluginManagement
{
    public class Main : IPlugin, IPluginI18n
    {
        ...
        private List<Result> ResultForInstallPlugin(Query query)
        {
            ...
            catch (WebException e)
            {
                ...
                context.API.ShowMsg("PluginManagement.ResultForInstallPlugin: Can't connect to Wow plugin website, check your conenction");
                Logger.WowError("Can't connect to Wow plugin website, check your conenction", e);
                return new List<Result>();
            }
            List<WowPluginResult> searchedPlugins;
            try
            {
                searchedPlugins = JsonConvert.DeserializeObject<List<WowPluginResult>>(json);
            }
            catch (JsonSerializationException e)
            {
                context.API.ShowMsg("PluginManagement.ResultForInstallPlugin: Coundn't parse api search results, Please update your Wow!");
                Logger.WowError("Coundn't parse api search results, Please update your Wow!", e);
                return results;
            }

            foreach (WowPluginResult r in searchedPlugins)
            {
                WowPluginResult r1 = r;
                results.Add(new Result
                {
                    ...
                    Action = c =>
                    {
                        ...
                        catch (WebException e)
                        {
                            ...
                            Logger.WowError($"download failed for <{r.name}>", e);
                            return false;
                        }
        ...
        private void UnInstallPlugin(PluginMetadata plugin)
        {
            ...
            if (MessageBox.Show(content, "Wow", MessageBoxButton.YesNo) == MessageBoxResult.Yes)
            {
