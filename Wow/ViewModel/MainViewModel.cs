...
namespace Wow.ViewModel
{
    public class MainViewModel : BaseModel, ISavable
    {
        ...
        private readonly WowJsonStorage<History> _historyItemsStorage;
        private readonly WowJsonStorage<UserSelectedRecord> _userSelectedRecordStorage;
        private readonly WowJsonStorage<TopMostRecord> _topMostRecordStorage;
        ...
        public MainViewModel(bool useUI = true)
        {
            ...
            _historyItemsStorage = new WowJsonStorage<History>();
            _userSelectedRecordStorage = new WowJsonStorage<UserSelectedRecord>();
            _topMostRecordStorage = new WowJsonStorage<TopMostRecord>();
        ...
        private void RegisterResultConsume()
        {
            ...
            Task.Run(() =>
            {
                while (true)
                {
                    ...
                    Logger.WowTrace($"start {startTime.Millisecond} end {currentTime.Millisecond}");
                    foreach (var update in updates)
                    {
                        Logger.WowTrace($"update name:{update.Metadata.Name} count:{update.Results.Count} query:{update.Query} token:{update.Token.IsCancellationRequested}");
                    ...
                    if (currentTime < viewExpired)
                    {
                        ...
                        Logger.WowTrace($"expired { viewExpired.Millisecond} span {span.TotalMilliseconds}");
                        Thread.Sleep(span);
                    }
                }
        ...
        {
            if (e.PropertyName == nameof(Settings.MaxResultsToShow) && !string.IsNullOrEmpty(_queryText))
            {
                Query();
            }
        }
        ...
        private void QueryResults()
        {
            if (_updateSource != null && !_updateSource.IsCancellationRequested)
            {
                ...
                Logger.WowDebug($"cancel init {_updateSource.Token.GetHashCode()} {Thread.CurrentThread.ManagedThreadId} {QueryText}");
            ...
            Task.Run(() =>
            {
                if (!string.IsNullOrEmpty(queryText))
                {
                    ...
                    if (query != null)
                    {
                        ...
                        {
                            Logger.WowTrace($"progressbar visible 1 {token.GetHashCode()} {token.IsCancellationRequested}  {Thread.CurrentThread.ManagedThreadId}  {query} {ProgressBarVisibility}");
                        ...
                        foreach (var plugin in plugins)
                        {
                            Task.Run(() =>
                            {
                                if (token.IsCancellationRequested)
                                {
                                    Logger.WowTrace($"canceled {token.GetHashCode()} {Thread.CurrentThread.ManagedThreadId}  {queryText} {plugin.Metadata.Name}");
                                    countdown.Signal();
                                    return;
                                }
                        ...
                        Task.Run(() =>
                        {
                            Logger.WowTrace($"progressbar visible 2 {token.GetHashCode()} {token.IsCancellationRequested}  {Thread.CurrentThread.ManagedThreadId}  {query} {ProgressBarVisibility}");
        ...
        /// <summary>
        /// Checks if Wow should ignore any hotkeys
        /// </summary>
        /// <returns></returns>
        ...
        public void UpdateResultView(List<ResultsForUpdate> updates)
        {
            ...
            {
                Logger.WowTrace($"{update.Metadata.Name}:{update.Query.RawQuery}");
