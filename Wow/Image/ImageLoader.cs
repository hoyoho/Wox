...
namespace Wow.Image
{
    public static class ImageLoader
    {
        ...
        {
            Logger.WowDebug($"load from disk {path}");
            ...
            if (path.StartsWith("data:", StringComparison.OrdinalIgnoreCase))
            {
                ...
                catch (Exception e)
                {
                    ...
                    Logger.WowError($"cannot load {path}", e);
            ...
            Logger.WowTrace($"{path} {subPath} {parent1} {parent2}");
            bool imageInsideWowDirectory = IsSubdirectory(parent1, subPath) || IsSubdirectory(parent2, subPath);
            if (normalImage && imageInsideWowDirectory)
            {
                try
                {
                    image = new BitmapImage(new Uri(path))
            ...
            if (File.Exists(path))
            {
                ...
                catch (ShellException e1)
                {
                    ...
                    {
                        Logger.WowError($"Failed to get thumbnail, first, {path}", e1);
                        Logger.WowError($"Failed to get thumbnail, second, {path}", e2);
                        image = GetErrorImage();
                        return image;
                    }
                }
        ...
        {
            Logger.WowDebug($"load begin {path}");
            var img = _cache.GetOrAdd(path, (string p) =>
            {
                ...
                catch (Exception e)
                {
                    ...
                    Logger.WowError($"cannot load image sync <{p}>", e);
                    return GetErrorImage();
                }
            });
            Logger.WowTrace($"load end {path}");
            return img;
        }
        ...
        internal static ImageSource Load(string path, Action<ImageSource> updateImageCallback, string title, string pluginID, string pluginDirectory)
        {
            ...
            var img = _cache.GetOrAdd(path, _defaultFileImage, (string p) =>
            {
                ...
                catch (Exception e)
                {
                    ...
                    e.Data.Add(nameof(p), p);
                    Logger.WowError($"cannot load image async <{p}>", e);
                    return GetErrorImage();
                }
            }, updateImageCallback
            );
