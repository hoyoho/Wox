    ...
    class ImageCache
    {
            ...
            Task.Run(() =>
            {
                while (true)
                {
                    ...
                    if (currentCount > _cacheLimit)
                    {
                        ...
                        Logger.WowDebug($"remove exceed: <{removeResult}> entry: <{min.Key}>");
                    }
                    else
                    {
        ...
        private void ExpirationCheck(object state)
        {
            try
            {
                ...
                Logger.WowDebug($"ExpirationCheck start {now}");
                ...
                foreach (KeyValuePair<string, CacheEntry> pair in pairs)
                {
                    ...
                    Logger.WowDebug($"remove expired: <{success}> entry: <{pair.Key}>");
                }
            }
            catch (Exception e)
            {
                e.Data.Add(nameof(state), state);
                Logger.WowError($"error check image cache with state: {state}", e);
            }
        }
