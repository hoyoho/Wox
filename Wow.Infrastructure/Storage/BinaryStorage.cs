    ...
    public class BinaryStorage<T>
    {
        ...
        public T TryLoad(T defaultData)
        {
            if (File.Exists(FilePath))
            {
                ...
                {
                    Logger.WowError($"Zero length cache file <{FilePath}>");
                    Save(defaultData);
                    return defaultData;
                }
            ...
            {
                Logger.WowInfo("Cache file not exist, load default data");
                Save(defaultData);
                return defaultData;
            }
        }
        ...
        private T Deserialize(FileStream stream, T defaultData)
        {
            ...
            {
                Logger.WowError($"Deserialize error for file <{FilePath}>", e);
                return defaultData;
            }
            finally
            {
        ...
        public void Save(T data)
        {
            using (var stream = new FileStream(FilePath, FileMode.Create))
            {
                ...
                {
                    Logger.WowError($"serialize error for file <{FilePath}>", e);
                }
            }
        }
    }
