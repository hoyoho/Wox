...
using static Wow.Infrastructure.StringMatcher;
...
namespace Wow.Infrastructure
{
    public class StringMatcher
    {
        ...
        public MatchResult FuzzyMatch(string query, string stringToCompare)
        {
            ...
            var match = CachedFuzzyMatch(queryWithoutCase, translated);

            // 副形态:拼音形态未命中时,再用原文(如中文原文)匹配,保证输入中文也能命中
            if (!match.Success && !string.Equals(translated, stringToCompare, StringComparison.Ordinal))
            {
                match = CachedFuzzyMatch(queryWithoutCase, stringToCompare);
            }

            return match;
        }

        private MatchResult CachedFuzzyMatch(string query, string candidate)
        {
            string key = $"{query}|{candidate}";
            MatchResult match = _cache[key] as MatchResult;
            if (match == null)
            {
                match = FuzzyMatchRecurrsive(query, candidate, 0, 0, new List<int>());
